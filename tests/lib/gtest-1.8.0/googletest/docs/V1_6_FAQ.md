...
## Why does Google Test support EXPECT\_EQ(NULL, view) and ASSERT\_EQ(NULL, view) but not EXPECT\_NE(NULL, view) and ASSERT\_NE(NULL, view)? ##
...
The need for `EXPECT_NE(NULL, view)` isn't nearly as strong. When the
assertion fails, you already know that `view` must be `NULL`, so it
doesn't add any information to print view in this case. That means
`EXPECT_TRUE(view ! NULL)` works just as well.

If we were to support `EXPECT_NE(NULL, view)`, for consistency we'll
have to support `EXPECT_NE(view, NULL)` as well, as unlike `EXPECT_EQ`,
